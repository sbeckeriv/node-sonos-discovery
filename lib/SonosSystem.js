...
function SonosSystem(settings) {
  ...
  let playerCache = {};
  let subCache = {};
  let fuzzyMatch = settings.fuzzyMatch;
  restart();
...
SonosSystem.prototype.getPlayer = function getPlayer(name) {
  var fuzzyMatch = this.fuzzyMatch;
  return this.players.find((player) => {
    player.roomName.toLowerCase() === name.toLowerCase() ||
      (fuzzyMatch &&
        Levenshtein( player.roomName.toLowerCase() , name.toLowerCase() ).distance() <= name.toLowerCase().length * 0.30
      )
  });
};
