...
function SonosSystem(settings) {
  ...
  let playerCache = {};
  let subCache = {};
  _this.fuzzyMatch = settings.fuzzyMatch;
  restart();
...
SonosSystem.prototype.getPlayer = function getPlayer(name) {
  var fuzzyMatches = this.fuzzyMatch;
  return this.players.find((player) => {
    var distance = (new Levenshtein(player.roomName.toLowerCase(), name.toLowerCase())).distance;
    var cleanName = name.replace(/^the\s*/i,"").toLowerCase();
    logger.debug(cleanName);
    return player.roomName.toLowerCase() === cleanName ||
      (distance <= name.length * 0.40);
  });
};
